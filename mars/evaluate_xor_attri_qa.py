...
for lang1, lang2 in [("en", "en"), ("ja", "ja"), ("ja", "en"), ("en", "ja")]:
    ppi_config = {
        ...
        "prediction_file_paths": [f"results_output_xor_attri_qa_{lang1}_{lang2}_Context_Relevance.json", f"results_output_xor_attri_qa_{lang1}_{lang2}_Answer_Relevance.json", f"results_output_xor_attri_qa_{lang1}_{lang2}_Answer_Faithfulness.json", f"results_output_xor_attri_qa_{lang1}_{lang2}_Language_Consistency.json"],
    }

    ares_module = ARES(ppi=ppi_config)
    results = ares_module.evaluate_RAG()
    print(results)
    json.dump(results, open(f"results_xor_attri_qa_{lang1}-{lang2}.json", "w"))
...
ppi_config = {
    ...
    "model_choice": "microsoft/mdeberta-v3-base",
    "assigned_batch_size": 32,
    "prediction_file_paths": ["results_output_xor_attri_qa_Context_Relevance.json", "results_output_xor_attri_qa_Answer_Relevance.json", "results_output_xor_attri_qa_Answer_Faithfulness.json", "results_output_xor_attri_qa_Language_Consistency.json"],
}

ares_module = ARES(ppi=ppi_config)
results = ares_module.evaluate_RAG()
print(results)
json.dump(results, open("results_xor_attri_qa_all.json", "w"))
